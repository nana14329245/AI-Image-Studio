...
import { UPSCALE_MAX_CREDITS, UPSCALE_MAX_OUTPUT_EDGE, planUpscale } from "@/lib/plans";
...
export default function UpscalePage() {
  const [source, setSource] = useState<Source | null>(null), [url, setUrl] = useState(""), [scale, setScale] = useState<2 | 4>(2), [enhancement, setEnhancement] = useState<"dehaze" | "vivid">("dehaze"), [result, setResult] = useState<string | null>(null), [generationId, setGenerationId] = useState<string | null>(null), [resultScale, setResultScale] = useState<2 | 4>(2), [view, setView] = useState<"original" | "result">("original"), [busy, setBusy] = useState(false), [loading, setLoading] = useState(false), [dragging, setDragging] = useState(false), [error, setError] = useState("");
  ...
  // Same function the server charges with, so the price shown is the price paid.
  const plan = source ? planUpscale(source.width, source.height, scale) : null;
  const resultPlan = source ? planUpscale(source.width, source.height, resultScale) : null;
  ...
  // Shrinking to 2048px loses nothing: 2× output is capped at UPSCALE_MAX_OUTPUT_EDGE, so the server would shrink it to the same size.
  async function chooseFile(file?: File) { if (!file || busy || loading) return; const ticket = ++selection.current; setLoading(true); setError(""); setResult(null); setGenerationId(null); setSource(null); setView("original"); try { const prepared = await prepareImageUpload(file, GENERATION_UPLOAD_MAX_EDGE); if (ticket === selection.current) setSource({ url: prepared.dataUrl, name: file.name, width: prepared.width, height: prepared.height }); } catch (e) { if (ticket === selection.current) setError(uploadErrorMessage(e)); } finally { if (ticket === selection.current) setLoading(false); } }
    ...
    <div className="page-wrap">
      <section className="grid border border-ink lg:grid-cols-[340px_minmax(0,1fr)]">
        <div className="border-b border-ink p-6 lg:border-b-0 lg:border-r">
          ...
          <div className="mt-7 border-t border-line-soft pt-6"><div className="mb-4 flex items-center justify-between"><h2 className="font-medium">อัตราขยาย</h2><span className="micro">02 / SCALE</span></div><div className="grid grid-cols-2 gap-3">{([2,4] as const).map(n=><button key={n} type="button" disabled={busy} aria-pressed={scale===n} onClick={()=>setScale(n)} className={`flex items-end justify-between border p-4 ${scale===n?"border-ink bg-ink text-paper":"border-line hover:bg-surface-hover"} ${focus}`}><span className="text-4xl font-medium">{n}×</span><span className="font-mono text-xs">{n*100}%</span></button>)}</div><dl className="my-5 flex justify-between text-sm"><dt className="text-muted-soft">ขนาดเป้าหมาย</dt><dd className="font-mono">{plan?`${plan.outputWidth} × ${plan.outputHeight} px`:"— × — px"}</dd></dl>{plan?.downscaled&&<p className="-mt-3 mb-5 text-xs leading-5 text-muted-soft">ภาพใหญ่เกินกำหนด ระบบจะย่อต้นฉบับให้ผลลัพธ์ด้านยาวไม่เกิน {UPSCALE_MAX_OUTPUT_EDGE}px</p>}
          ...
          <p className="mt-3 text-center font-mono text-xs text-muted">{plan?`ใช้ ${plan.credits} เครดิต`:`คิดเครดิตตามขนาดภาพ สูงสุด ${UPSCALE_MAX_CREDITS} เครดิต`}</p>
